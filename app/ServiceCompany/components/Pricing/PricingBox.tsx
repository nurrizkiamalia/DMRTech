                ...
                {pricingdata.map((item, index) => (
                    <div key={index} className={`pricing-box ${pricingBoxStyle} `}>
                        <div className={` ${item.popular === true ? "block" : "hidden"} bg-dspPurple w-fit p-2 absolute top-10 -right-2`}>
                            <p className="uppercase font-bold text-pXL text-white">Best Offer</p>
                        </div>
                        <div>
                        ...
                        <button className="border-2 bg-dspLimeGreen border-black hover:border-dspLimeGreen py-3 rounded-3xl text-black font-bold hover:bg-dspBlack hover:text-white ">Start with {item.title}</button>
                    </div>
                ))}
            </div>
        </>
